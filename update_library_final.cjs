...
// 2. Change addClip
// Let's use AST-safe replace for addClip using string indexOf
const addClipStart = content.indexOf('const addClip = () => {');
const fastClipStart = content.indexOf('const handleFastAddClip =');

const addClipCode = `const addClip = async () => {
...
content = content.substring(0, addClipStart) + addClipCode + content.substring(fastClipStart);

// 3. Add handleSaveClip
const updateClipStart = content.indexOf('const updateClip =');
const handlePlayClipStart = content.indexOf('const handlePlayClip =');
const updateClipCode = `const updateClip = (clipId: string, updates: Partial<OpponentVideoClip>) => {
  ...
  `;
content = content.substring(0, updateClipStart) + updateClipCode + content.substring(handlePlayClipStart);

// 4. Robust Render Replacement
                    ...
                    (() => {
                      ...
                      return Object.entries(grouped).map(([cat, clipsInCat]) => (
                        <div key={cat} className="space-y-2">
                          ...
                          {clipsInCat.map(clip => (
                            <div key={clip.id} className="bg-black border border-brand-black-border rounded-xl p-3 space-y-2.5 group hover:border-brand-red-600/50 transition-colors">
                                  ...
                                  {!readOnly && (
                                    <>
                                      <button
                                        ...
                                        className="relative p-1.5 text-brand-gray-muted hover:text-brand-red-500 hover:bg-brand-red-600/10 rounded transition-colors"
