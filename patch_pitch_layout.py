...
# We need to extract the whole pitch logic block from ) : viewMode === 'pitch' ? ( to ) : viewMode === 'grid' ? (
...
pitch_logic_new = """          ) : viewMode === 'pitch' ? (
            <div className="w-full bg-[#469A33] rounded-2xl border-4 border-[#2D6A1F] p-4 sm:p-6 md:p-10 relative min-h-[700px] shadow-2xl overflow-hidden flex flex-col md:flex-row gap-2 md:gap-4" style={{ backgroundImage: 'repeating-linear-gradient(0deg, transparent, transparent 50px, rgba(0,0,0,0.06) 50px, rgba(0,0,0,0.06) 100px)' }}>
              ...
              {(() => {
                const getPlayerZone = (pos: string) => {
                  ...
                  let side = 'center';
                  if (p.includes('izquierd') || p.includes(' ei') || p.includes(' li')) side = 'left';
                  else if (p.includes('derech') || p.includes(' ed') || p.includes(' ld')) side = 'right';
                ...
                const renderPlayerCard = (player: any) => {
                  ...
                  return (
                    <div 
                      ...
                      className={`flex flex-col items-center cursor-pointer group hover:scale-110 transition-transform w-[60px] md:w-[70px] m-1 ${isSelected ? 'scale-110 ring-4 ring-brand-red-600 rounded-lg p-1 bg-black/20' : ''}`}
                ...
                return ['GK', 'DEF', 'MID', 'FWD'].map((lineCode) => {
                  ...
                  return (
                    <div key={lineCode} className={`flex-1 flex flex-row md:flex-col w-full md:w-auto h-auto md:h-full justify-between items-center relative z-10 ${orderClass}`}>
                      
                      {/* Left Side (Top on PC, Left on Mobile) */}
                      <div className="flex-1 flex flex-row md:flex-col justify-start md:justify-start items-center w-full h-full p-1">
                        {leftPlayers.map(renderPlayerCard)}
                      </div>

                      {/* Center */}
                      <div className="flex-[2] flex flex-row md:flex-col flex-wrap justify-center items-center w-full h-full p-1">
                        {centerPlayers.map(renderPlayerCard)}
                      </div>

                      {/* Right Side (Bottom on PC, Right on Mobile) */}
                      <div className="flex-1 flex flex-row md:flex-col justify-end md:justify-end items-center w-full h-full p-1">
...
print("Pitch layout completely revamped!")
