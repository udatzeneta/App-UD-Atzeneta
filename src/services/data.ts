...
import { Training, Match, Team, PlayerMatchStats, Fine, PointLog, ScoutingPlayer, OpponentAnalysis, Settings, TrainingAttendance, TacticalBoard, Player, PlayerWeight, PlayerPhysioRecord, PlayerInjury, SocialEvent, TrainingTask, TrainingSessionTask, Profile, FFCVSanction } from '../types';
import { isSameTeam, normalizeTeamName } from '../utils/teamUtils';
import { computeFFCVTeamStats, computeFFCVLeagueRankings, filterFFCVMatchesForTeam, filterScoutingForTeam } from '../utils/ffcvAggregation';
...
const isSamePlayer = (a?: string | null, b?: string | null): boolean => {
  if (!a || !b) return false;
  ...
  return matchCount >= 2 || (wordsA.length === 1 && matchCount === 1);
};
...
export const dataService = {
  ...
  async getPreviousSeasonScoutingForPlayers(playerNames: string[]): Promise<ScoutingPlayer[]> {
    if (!playerNames || playerNames.length === 0) return [];
    const uniqueNames = Array.from(new Set(playerNames.filter(Boolean)));
    if (uniqueNames.length === 0) return [];

    if (isMockMode) {
      const all = MockDatabase.getScouting();
      return all.filter((s: any) =>
        (s.season === '2025-2026' || s.season === '2025/2026') &&
        uniqueNames.some(name => isSamePlayer(s.player_name, name))
      );
    }

    try {
      const chunkSize = 50;
      let allFound: ScoutingPlayer[] = [];
      for (let i = 0; i < uniqueNames.length; i += chunkSize) {
        ...
        const { data, error } = await supabase
          ...
          .in('season', ['2025-2026', '2025/2026']);

        if (!error && data) {
          allFound = allFound.concat(data as ScoutingPlayer[]);
        }
      }
      return allFound;
    } catch (err) {
      console.warn('Error en getPreviousSeasonScoutingForPlayers:', err);
      return [];
    }
  },
  ...
  async getOpponentFFCVTeamStats(opponentName: string) {
    ...
    if (!isMockMode) {
      const { data } = await supabase
        ...
        .eq('played', true);
      ffcvMatches = filterFFCVMatchesForTeam(data || [], opponentName);

      // El scouting solo aporta disciplina (tarjetas), nunca resultados.
      const { data: scData } = await supabase.from('scouting').select('*');
      scoutingPlayers = filterScoutingForTeam((scData || []) as ScoutingPlayer[], opponentName);
    } else {
      scoutingPlayers = filterScoutingForTeam(MockDatabase.getScouting() as ScoutingPlayer[], opponentName);
    }

    return computeFFCVTeamStats(opponentName, ffcvMatches, scoutingPlayers);
  },

  async getOpponentFFCVLeagueRankings(opponentName: string) {
    if (!opponentName) return null;
    const teamStats = await this.getOpponentFFCVTeamStats(opponentName);
    return computeFFCVLeagueRankings(opponentName, teamStats);
  },

  // ===================== Enlaces públicos de presentaciones =====================
  // El token es la credencial del enlace; la presentación se resuelve en el
  // servidor (/api/shared-presentation), nunca con la anon key del visitante.

  async getOrCreatePresentationShare(
    analysisId: string,
    presentationId: string,
    meta?: { title?: string; opponent?: string },
  ): Promise<{ token: string; created_at?: string }> {
    if (isMockMode) {
      throw new Error('Los enlaces compartidos no están disponibles en modo demostración.');
    }

    // Reutilizamos el enlace vivo que ya exista: así el que ya circula por
    // WhatsApp sigue siendo válido en vez de quedarse huérfano.
    const { data: existing } = await supabase
    ...
    if (existing && existing.length > 0) {
      return existing[0] as { token: string; created_at?: string };
    }
    ...
    if (error) throw error;
    return data as { token: string; created_at?: string };
  },

  async revokePresentationShare(token: string): Promise<void> {
  ...
  getFFCVSanctions: async (season?: string): Promise<FFCVSanction[]> => {
    try {
      const { data, error } = await supabase
        .from('ffcv_sanctions')
        .select('*')
      ...
      if (!error && data && data.length > 0) {
        if (season) {
          return data.filter((s: FFCVSanction) => !s.season || s.season === season);
        }
        return data as FFCVSanction[];
      }
    } catch (e) {
      console.warn('Error querying ffcv_sanctions from supabase, falling back to local json:', e);
    }
    ...
    try {
      ...
      console.warn('Error loading local ffcv_sanctions.json:', e);
    }
