...
export const Matches: React.FC = () => {
  ...
  const teamFilter = (user?.role_id === 1 || user?.role_id === 4 || user?.role_id === 2 || (user?.availableContexts && user.availableContexts.length > 0)) ? filterTeam : user?.team_category;
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      team_category: teamFilter || filterTeam
    };
      ...
      >
        <div className="space-y-4">
          ...
          {squadModalMode === 'view' ? (
            ...
            {/* --- FILA 2: JUGADORES (Ancho completo) --- */}
            <div className="flex flex-col max-h-[450px]">
              ...
              <div className="flex border-b border-brand-black-border/60 bg-brand-black-card/40 border-l border-r">
                <button
                  type="button"
                  onClick={() => setSquadTab('primer_equipo')}
                  className={`flex-1 py-2 text-[11px] font-bold transition-colors border-b-2 ${squadTab === 'primer_equipo' ? 'border-brand-red-600 text-brand-red-600' : 'border-transparent text-brand-gray-muted hover:text-brand-gray-light'}`}
                >
                  Primer Equipo
                </button>
                <button
                  type="button"
                  onClick={() => setSquadTab('juvenil')}
                  className={`flex-1 py-2 text-[11px] font-bold transition-colors border-b-2 ${squadTab === 'juvenil' ? 'border-brand-red-600 text-brand-red-600' : 'border-transparent text-brand-gray-muted hover:text-brand-gray-light'}`}
                >
                  Juvenil
                </button>
              </div>
              <div className="flex-1 overflow-y-auto pr-1 no-scrollbar border border-brand-black-border/60 p-2 rounded-b-xl bg-brand-black-card/40 border-t-0">
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-1.5">
                  {dbPlayers.filter(p => squadTab === 'primer_equipo' ? p.team_category !== 'Juvenil' : p.team_category === 'Juvenil').length === 0 ? (
                    <div className="col-span-full text-center py-8 text-brand-gray-muted text-xs italic">
                      No hay jugadores en esta categoría.
                    </div>
                  ) : (
                    dbPlayers
                      .filter(p => squadTab === 'primer_equipo' ? p.team_category !== 'Juvenil' : p.team_category === 'Juvenil')
                      .map((player) => {
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-1 min-w-0 flex-1">
                            ...
                            <div className="flex-1 flex flex-col justify-center py-0.5">
                              <div className="flex items-center gap-1.5 mb-0.5">
                                ...
                                {isBaja && (
                                  <span className="text-[8px] font-black bg-brand-red-600 text-white px-1 py-0.5 rounded uppercase shrink-0 mt-0.5">
                                    Baja
                                  </span>
                                )}
                              </div>
                          ...
                          <div className="flex self-start mt-1 shrink-0 pl-1">
            ...
            <div className="bg-brand-black-card/40 p-4 rounded-xl border border-brand-black-border/60">
              ...
              <div className="flex flex-col md:flex-row items-center justify-center gap-8 md:gap-16">
                ...
                <div className="flex flex-col gap-4 min-w-[200px]">
                  {/* Camiseta */}
                  <div className="flex flex-col gap-1.5 bg-brand-black/25 p-3 rounded border border-brand-black-border/30">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        type="color"
                        className="w-6 h-6 rounded cursor-pointer border border-brand-black-border bg-transparent p-0 ml-2"
                        value={kitShirtColor}
